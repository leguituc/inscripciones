<div>
    ...
    >Grupo Familiar ({{ $parientes->total() + 1 }})
        <x-slot:botones>
    ...
    <div class="recuadro">
        @if($parientes->count()==0)
            <div class="flex my-3 border dark:border-yellow-100 rounded p-4">
                <span class="self-center text-4xl mdi mdi-alert-octagon-outline mr-2"></span>
                <span class="self-center">No se han encontrado miembros para el grupo familiar.</span>
            </div>
        @else
            @foreach($parientes as $pariente)
                <div class="my-3 border dark:border-yellow-100 rounded p-4 grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div>
                        ...
                        <span
                            class="text-xl dark:text-yellow-50">{{ number_format($pariente->dni, '0', ',', '.') }}</span>
                    </div>
                    <!-- INGRESOS -->
                    <div>
